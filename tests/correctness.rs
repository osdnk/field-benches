...
fn f162_aos4_v0() {
    ...
    for _ in 0..256 {
        let a: Vec<[u64; 3]> = (0..4).map(|_| rand162(&mut r)).collect();
        let b: Vec<[u64; 3]> = (0..4).map(|_| rand162(&mut r)).collect();
        let mut q01 = [0u64; 8];
        let mut q2 = [0u64; 8];
        let mut p01 = [0u64; 8];
        let mut p2 = [0u64; 8];
        ...
        let out = unsafe {
            f162::mul_aos4_v0(
                f162::Aos4 { q01: from_arr(q01), q2: from_arr(q2) },
                f162::Aos4 { q01: from_arr(p01), q2: from_arr(p2) },
            )
        };
        let o01 = to_arr(out.q01);
        let o2 = to_arr(out.q2);
        for i in 0..4 {
            let want = reference::mul162(a[i], b[i]);
            assert_eq!([o01[2 * i], o01[2 * i + 1], o2[2 * i]], want, "lane {i}");
        }
    }
}
...
fn f162_aos4_v1() {
    ...
    for _ in 0..256 {
        ...
        for i in 0..4 {
            let want = reference::mul162(a[i], b[i]);
            assert_eq!([o01[2 * i], o01[2 * i + 1], o2[2 * i]], want, "lane {i}");
            assert_eq!(o2[2 * i + 1], want[2], "dup lane {i}");
        }
    }
}
